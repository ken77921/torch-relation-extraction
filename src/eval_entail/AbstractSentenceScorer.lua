...
package.path = package.path .. ";src/?.lua;src/nn-modules/?.lua;src/eval_entail/?.lua"
...
function AbstractSentenceScorer:process_file(pattern_map, tac_map, dictionary)
    ...
    for line in io.lines(self.params.candidates) do
        local pattern_s, pattern_g, pattern_s_tensor, pattern_g_tensor, seq_len_s, seq_len_g = self:process_line(line, pattern_map, tac_map, dictionary)
        --print(pattern_tensor)
        if seq_len_s <= self.params.maxSeq and seq_len_s > 0 and seq_len_g <= self.params.maxSeq and seq_len_g > 0 then
            -- max_seq = math.max(seq_len, max_seq)
            if not pattern_set[pattern_s] then
                if not data[seq_len_s] then data[seq_len_s] = {pattern={}, pattern_tensor={}} end
                local seq_len_data_s = data[seq_len_s]
                table.insert(seq_len_data_s.pattern, pattern_s)
                table.insert(seq_len_data_s.pattern_tensor, pattern_s_tensor)
                pattern_set[pattern_s] = 0
            end
            if not pattern_set[pattern_g] then
                if not data[seq_len_g] then data[seq_len_g] = {pattern={}, pattern_tensor={}} end
                local seq_len_data_g = data[seq_len_g]
                table.insert(seq_len_data_g.pattern, pattern_g)
                table.insert(seq_len_data_g.pattern_tensor, pattern_g_tensor)
                pattern_set[pattern_g] = 0
            end
...
function AbstractSentenceScorer:process_line(line, pattern_map, tac_map, dictionary)
    local pattern_s, pattern_g = string.match(line, "([^\t]+)\t([^\t]+)")

    --print(line)
    -- if self.params.normalizeDigits ~= '' and not self.params.fullPath then pattern_s = pattern_s:gsub("%d", self.params.normalizeDigits) end
    -- if self.params.normalizeDigits ~= '' and not self.params.fullPath then pattern_g = pattern_g:gsub("%d", self.params.normalizeDigits) end
    ...
    -- end
    pattern_s_tensor, seq_len_s = self:token_tensor(pattern_s, pattern_map, dictionary)
    pattern_g_tensor, seq_len_g = self:token_tensor(pattern_g, pattern_map, dictionary)

    pattern_s_tensor = pattern_s_tensor:view(1, pattern_s_tensor:size(1))
    pattern_g_tensor = pattern_g_tensor:view(1, pattern_g_tensor:size(1))
    --print(pattern_tensor:size())
    -- tac_tensor = tac_tensor:view(1, tac_tensor:size(1))
    -- local enitity_pair = query_id .. '\t' .. sf_2
    -- local out_line = pattern_s .. '\t' .. pattern_g .. '\t' 
    ...
    return pattern_s, pattern_g, pattern_s_tensor, pattern_g_tensor, seq_len_s, seq_len_g
end
...
function AbstractSentenceScorer:token_tensor(pattern_rel, vocab_map, dictionary)
    -- local idx = 0
    local token_ids = {}
    local tokens = {}

    local first_arg = '$ARG1'
    local second_arg = '$ARG2'
    ...
    for token in string.gmatch(pattern_rel, "[^" .. self.params.delim .. "]+") do
        ...
        if self.params.normalizeDigits ~= '' then token = token:gsub("%d", self.params.normalizeDigits) end
...
function AbstractSentenceScorer:score_data(data, ep_tac_scores)
    ...
    for seq_len, sub_data in pairs(data) do
        if torch.type(sub_data) == 'table' then
            ...
            for i = 1, pattern_encoded:size(1) do
                pattern = sub_data.pattern[i]
                pattern_encoded_i = pattern_encoded[i]
                out_file:write(pattern .. '\t') 
                for j = 1, pattern_encoded_i:size(1) do
                    out_file:write(pattern_encoded_i[j] .. ' ')
                end
                out_file:write('\n')
            end
